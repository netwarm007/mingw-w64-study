...
bool SystemClass::Initialize()
{
	...
	if(!result)
	{
		MessageBox(m_hwnd, _T("Could not initialize the window."), _T("Error"), MB_OK);
		return false;
	}
...
bool SystemClass::InitializeWindows(OpenGLClass* OpenGL, int& screenWidth, int& screenHeight)
{
	...
	if(!result)
	{
		MessageBox(m_hwnd, _T("Could not initialize the OpenGL extensions."), _T("Error"), MB_OK);
		return false;
	}
	...
	if(!result)
	{
		MessageBox(m_hwnd, _T("Could not initialize OpenGL, check if video card supports OpenGL 4.0."), _T("Error"), MB_OK);
		return false;
	}
